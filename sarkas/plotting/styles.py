...
# MSU Color palette
MSU_COLORS = [
    '#18453b', '#f08521', '#008183', '#6e005f', '#d1de3f', 
    '#0db14b', '#c89a58', '#535054', '#909ab7', '#e8d9b5', 
    '#94ae4a', '#cb5a28'
]
...
def _create_msu_template():
    ...
    template.data.scatter = [
        go.Scatter(line=dict(width=2), marker=dict(size=6))
    ]
    template.data.bar = [
        go.Bar(marker=dict(line=dict(width=0.8, color='black')))
    ]
...
def use_msu_style_matplotlib():
    ...
    if os.path.exists(MSU_MPLSTYLE_PATH):
        ...
        raise FileNotFoundError(
            f"MSU matplotlib style file not found at {MSU_MPLSTYLE_PATH}"
        )
...
def use_pub_style_matplotlib():
    ...
    if os.path.exists(PUB_MPLSTYLE_PATH):
        ...
        raise FileNotFoundError(
            f"PUB matplotlib style file not found at {PUB_MPLSTYLE_PATH}"
        )