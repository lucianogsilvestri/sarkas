...
from numpy import ndarray, zeros, bincount
from numba import jit


@jit(nopython=True)
def species_sum(per_particle_array: ndarray, species_id: ndarray, 
    ...
    if per_particle_array.ndim == 1:
        # Scalar quantities
        result = zeros(num_species, dtype=per_particle_array.dtype)
        ...
        result = zeros(shape, dtype=per_particle_array.dtype)
        for i in range(len(per_particle_array)):
            sp_id = int(species_id[i])
            result[sp_id] += per_particle_array[i]
...
@jit(nopython=True)
def species_mean(per_particle_array: ndarray, species_id: ndarray, 
                num_species: int) -> ndarray:
    ...
    if num_species == 0 or per_particle_array.size == 0 or species_id.size == 0:
        if per_particle_array.ndim == 1:
            return zeros(num_species, dtype=per_particle_array.dtype)
        else:
            shape = (num_species,) + per_particle_array.shape[1:]
            return zeros(shape, dtype=per_particle_array.dtype)
    # Get species counts
...
@jit(nopython=True)
def species_weighted_sum(per_particle_array: ndarray, weights: ndarray, 
...
@jit(nopython=True)
def species_variance(per_particle_array: ndarray, species_id: ndarray, 
                    num_species: int) -> ndarray:
    ...
    .. code-block:: python
        ...
        velocities = random.randn(1000, 3)
        speed = linalg.norm(velocities, axis=1)
        species_id = random.randint(0, 2, 1000)
...
@jit(nopython=True)
def get_species_counts(species_id: ndarray, num_species: int) -> ndarray:
    ...
    """
    return bincount(species_id, minlength=num_species)


def validate_species_aggregation(per_particle_array: ndarray, species_id: ndarray, 
    ...
    if any(species_id < 0) or any(species_id >= num_species):
        return False
    
    # Check for NaN or infinite values
    if any(isnan(per_particle_array)) or any(isinf(per_particle_array)):
        return False
...
def bincount_vector(species_id, vectors, num_species):
    result = zeros((num_species, vectors.shape[1]), dtype=vectors.dtype)
    for j in range(vectors.shape[1]):
        result[:, j] = bincount(species_id, weights=vectors[:, j], minlength=num_species)
    return result


@jit(nopython=True)
def bincount_tensor(species_id, tensors, num_species):
    result = zeros((num_species, tensors.shape[1], tensors.shape[2]), dtype=tensors.dtype)
    for i in range(tensors.shape[1]):
        for j in range(tensors.shape[2]):
            result[:, i, j] = bincount(species_id, weights=tensors[:, i, j], minlength=num_species)
    return result
...
def fast_species_sum(per_particle_array, species_id, num_species):
    ...
    """
    if per_particle_array.ndim == 1:
        return species_sum(per_particle_array, species_id, num_species)
